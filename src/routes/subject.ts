...
router.put("/:id", requireAdmin, async (req, res) => {
    try {
        ...
        const [duplicateSubject] = await db
            .select({ id: subjects.id })
            .from(subjects)
            .where(and(eq(subjects.code, code), sql`${subjects.id} <> ${subjectId}`));
...
router.delete("/:id", requireAdmin, async (req, res) => {
    try {
        ...
        if (linkedClass) {
            return res.status(409).json({ error: "Subject has classes and cannot be deleted" });
        }
